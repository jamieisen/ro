...
class L3GD20(Adafruit_I2C):
    ...
    def __init__(self, busnum=-1, debug=False, rng=L3GD20_range.R250DPS):
        ...
        logging.info("GYRO:\tConnecting to device")
    ...
    def readX(self):
        list = self.gyro.readList(self.L3GD20_REGISTER_OUT_X_L | 0x80, 6)
        return self.gyro16(list,0)*self.sensitivity

    def readY(self):
        list = self.gyro.readList(self.L3GD20_REGISTER_OUT_X_L | 0x80, 6)
        return self.gyro16(list,2)*self.sensitivity

    def readZ(self):
        list = self.gyro.readList(self.L3GD20_REGISTER_OUT_X_L | 0x80, 6)
        return self.gyro16(list,4)*self.sensitivity
...
if __name__ == "__main__":
    ...
    while True:
        print "(X: "+str(int(l3.readX()))+",Y: "+str(int(l3.readY()))+",Z: "+str(int(l3.readZ()))+")"
        sleep(1)
